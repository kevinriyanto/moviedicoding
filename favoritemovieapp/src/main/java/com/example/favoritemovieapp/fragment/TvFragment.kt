...
class TvFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        view.favorite_consum_list_tv.layoutManager = LinearLayoutManager(context)
        view.favorite_consum_list_tv.setHasFixedSize(true)
        adapter = ListMovieAdapter {

        }
        view.favorite_consum_list_tv.adapter = adapter
        ...
        requireContext().contentResolver?.registerContentObserver(DatabaseContract.FavoriteTVColumns.CONTENT_URI, true, myObserver)
    ...
    private fun loadMovieAsync() {
        GlobalScope.launch(Dispatchers.Main) {
            val deferredNotes = async(Dispatchers.IO) {
                cursor = requireContext().contentResolver?.query(DatabaseContract.FavoriteTVColumns.CONTENT_URI, null, null, null, null) as Cursor
